...
export function EnumSelect({value,options,onChange,tone="dim",free,label,compact,disabled=false}:{
 ...
 const listId=useId();
 if(free)return <>
  <input className={"input"+(compact?" input-compact":"")} list={listId} value={value??""} aria-label={label} disabled={disabled}
   onChange={e=>onChange(e.target.value)}/>
  <datalist id={listId}>{options.map(o=><option key={o} value={o}/>)}</datalist>
 </>;
 return <select className={compact?"badge badge-select tone-"+tone:"input"} value={value??""} aria-label={label} disabled={disabled}
