...
export function ProjectView({projectId}:{projectId:string}){
 ...
 const newEntity=(key:ProjectEntityKey,preset:Record<string,unknown>={})=>
  setComposer({key,seed:{...ENTITIES[key].newDefaults({userId:data.userId,projectId}),...preset}});
 ...
 return <div className="project-view">
  <header className="project-head">
   ...
   <div className="project-head-actions">
    ...
    <button className="secondary" onClick={()=>setManageStages(true)}><FolderKanban/>Manage stages</button>
    <button className="primary" onClick={()=>newEntity("tasks",{stage_id:stages[0]?.id})}><Plus/>Add work</button>
   </div>
  </header>
  ...
  {manageStages&&<StageManager project={project} stages={stages} tasks={projectTasks} onClose={()=>setManageStages(false)}/>} 
  {person&&<PersonEditor person={person} onClose={()=>setPerson(null)}/>} 
  {composer&&<Composer def={ENTITIES[composer.key]} seed={composer.seed} onClose={()=>setComposer(null)} onSubmit={submitEntity}/>} 
  {drawer&&drawerRow&&<Drawer def={ENTITIES[drawer.entity]} row={drawerRow} onClose={ui.closeDrawer}/>} 
 </div>;
}

function ProjectOverview({project}:{project:Project}){
 const {userId,projects,projectLinks,projectFields,tables,tags}=useData();
 ...
 const save=(patch:Record<string,unknown>)=>void projects.update(project.id,patch);
 const addLink=async()=>{
  const label=linkLabel.trim(),url=linkUrl.trim();if(!label||!safeHttp(url))return;
  await projectLinks.insert({user_id:userId,project_id:project.id,label,url,position:(links.at(-1)?.position??-1)+1});
  setLinkLabel("");setLinkUrl("");
 };
 const addField=async()=>{
  const label=fieldLabel.trim();if(!label)return;
  await projectFields.insert({user_id:userId,project_id:project.id,label,value:fieldValue.trim()||null,
  ...
  <section className="panel project-summary-panel">
   ...
   <div className="field"><label className="field-label" htmlFor="project-objective">Objective</label>
    <textarea id="project-objective" className="input" rows={3} defaultValue={project.objective??""}
     placeholder="What result should this project produce?" onBlur={e=>{if(e.target.value!==project.objective)save({objective:e.target.value.trim()||null})}}/></div>
   <div className="field"><label className="field-label" htmlFor="project-description">Description</label>
    <textarea id="project-description" className="input" rows={3} defaultValue={project.description??""}
     placeholder="Scientific context, scope and decisions" onBlur={e=>{if(e.target.value!==project.description)save({description:e.target.value.trim()||null})}}/></div>
   <div className="project-detail-grid">
    ...
    <div className="field"><label className="field-label" htmlFor="project-start">Start date</label>
     <input id="project-start" className="input" type="date" value={project.start_date??""} onChange={e=>save({start_date:e.target.value||null})}/></div>
    <div className="field"><label className="field-label" htmlFor="project-target">Target date</label>
     <input id="project-target" className="input" type="date" value={project.target_date??""} onChange={e=>save({target_date:e.target.value||null})}/></div>
   </div>
   <div className="field field-wide"><span className="field-label">Project tags</span>
    <TagEditor tags={tags.tags} selected={selectedTags} label="Project tags"
     onToggle={id=>void tags.setFor("project",project.id,selectedTags.includes(id)?selectedTags.filter(x=>x!==id):[...selectedTags,id])}
     onCreate={async name=>{const tag=await tags.create(name);if(tag)void tags.setFor("project",project.id,[...selectedTags,tag.id])}}/></div>
  </section>
   ...
   <section className="panel">
    ...
    <div className="resource-list">
     {links.map(link=><div className="resource-row" key={link.id}>
      ...
      <button className="icon-button" onClick={()=>void projectLinks.remove(link.id)} aria-label={`Delete ${link.label}`}><Trash2/></button>
     </div>)}
     {!links.length&&<p className="muted-note">Add Drive folders, repositories, protocols or datasets.</p>}
    </div>
    <div className="compact-add-grid">
   ...
   <section className="panel">
    ...
    <div className="resource-list">
     {fields.map(field=><div className="custom-field-row" key={field.id}>
      ...
      <button className="icon-button" onClick={()=>void projectFields.remove(field.id)} aria-label={`Delete ${field.label}`}><Trash2/></button>
     </div>)}
    </div>
    <div className="compact-add-grid">
...
function ProjectBoard({project,stages,tasks,onAdd,onOpen,onManage}:{
 project:Project;stages:ProjectStage[];tasks:Row[];onAdd(stageId:string|null):void;onOpen(id:string):void;onManage():void;
}){
 ...
 return <section className="project-workspace">
  <div className="project-section-head"><div><h2>Custom workflow</h2><p className="subtitle">Each column belongs only to {project.name}.</p></div>
   <button className="secondary" onClick={onManage}><FolderKanban/>Add or reorder stages</button></div>
  <div className="project-board">
   {columns.map(stage=>{
    ...
    return <section className="project-column" key={stage.id||"unsorted"}
     onDragOver={e=>{if(stage.id&&e.dataTransfer.types.includes(TASK_MIME)){e.preventDefault();e.dataTransfer.dropEffect="move"}}}
     onDrop={e=>{e.preventDefault();const id=e.dataTransfer.getData(TASK_MIME);if(stage.id&&tasks.some(t=>t.id===id))move(id,stage.id)}}>
     <header className="project-column-head"><span className={"column-name tint-"+(stage.color??"slate")}>{stage.name}</span><b>{rows.length}</b>
      {stage.id&&<button className="icon-button" onClick={()=>onAdd(stage.id)} aria-label={`Add work to ${stage.name}`}><Plus/></button>}</header>
     <div className="project-task-list">
      {rows.map(task=>{
       ...
       return <article className="project-task" key={task.id} draggable onDragStart={e=>{e.dataTransfer.setData(TASK_MIME,task.id);e.dataTransfer.effectAllowed="move"}}
        ...
        <div className="project-task-actions">
         <select className="input input-compact" value={task.stage_id??""} aria-label={`Move ${task.title}`}
...
function ProjectPublications({project,direct,related,onAdd,onOpen}:{project:Project;direct:Row[];related:Row[];onAdd():void;onOpen(id:string):void}){
 ...
 const card=(paper:Row,relation:"project"|"tag")=>{
  ...
  return <article className="project-paper" key={paper.id} onClick={e=>{if(!(e.target as HTMLElement).closest("button,a"))onOpen(paper.id)}}>
   ...
   <div className="paper-progress"><span>{nodes.length?`${done} of ${nodes.length} custom steps complete`:"No custom steps yet"}</span>
    {relation==="tag"&&<button className="link-button" onClick={()=>void tables.publications.update(paper.id,{project_id:project.id})}>Link directly</button>}</div>
  </article>;
 };
 return <section className="project-workspace">
  <div className="project-section-head"><div><h2>Publications</h2><p className="subtitle">Direct project papers and papers related through shared tags.</p></div>
   <button className="primary" onClick={onAdd}><Plus/>New publication</button></div>
  <div className="project-paper-grid">{direct.map(p=>card(p,"project"))}{related.map(p=>card(p,"tag"))}</div>
  {!direct.length&&!related.length&&<div className="empty-state"><FlaskConical/><p>No publications are linked to this project or its tags.</p><button className="primary" onClick={onAdd}>Add publication</button></div>}
 </section>;
}

function ProjectReads({project,rows,onAdd,onOpen}:{project:Project;rows:Row[];onAdd():void;onOpen(id:string):void}){
 ...
 return <section className="project-workspace">
  <div className="project-section-head"><div><h2>Project Reads</h2><p className="subtitle">Links saved from the web or Telegram for {project.name}.</p></div>
   <button className="primary" onClick={onAdd}><Plus/>New Read</button></div>
  {inbox.length>0&&<div className="assign-read"><select className="input" value={existing} onChange={e=>setExisting(e.target.value)} aria-label="Choose an Inbox read">
...
function ProjectPeople({projectId,onAdd,onEdit}:{projectId:string;onAdd():void;onEdit(person:Person):void}){
 ...
 return <section className="project-workspace">
  <div className="project-section-head"><div><h2>Private people directory</h2><p className="subtitle">These are labels and contact details only. No invitation is sent and they cannot see the workspace.</p></div>
   <button className="primary" onClick={onAdd}><Plus/>Add person</button></div>
  <div className="people-grid">{ordered.map(person=>{
   ...
   return <article className="person-card" key={person.id}>
    ...
    <div className="person-actions"><button className="icon-button" onClick={()=>onEdit(person)} aria-label={`Edit ${person.name}`}><Pencil/></button>
  ...
  {!ordered.length&&<div className="empty-state"><Users/><p>Add someone as a private assignee. They will not receive an account or notification.</p></div>}
 </section>;
}

function StageManager({project,stages,tasks,onClose}:{project:Project;stages:ProjectStage[];tasks:Row[];onClose():void}){
 const {userId,projectStages}=useData();
 ...
 const add=async()=>{const title=name.trim();if(!title)return;
  const row=await projectStages.insert({user_id:userId,project_id:project.id,name:title,category,color,position:(stages.at(-1)?.position??-1)+1});
  if(row)setName("");
 };
 ...
 return <Modal kicker="Project workflow" title={`Stages for ${project.name}`} size="lg" onClose={onClose}
  ...
  <div className="stage-list">{stages.map((stage,index)=>{
   ...
   return <div className="stage-row" key={stage.id}>
    ...
    <button className="icon-button" disabled={used>0} title={used?"Move its cards before deleting":"Delete stage"}
