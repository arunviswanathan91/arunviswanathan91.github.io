...
export function SettingsView(){
 ...
 const colourOptions=[{value:"",label:"No colour"},...TAG_COLORS.map(value=>({value,label:value}))];
  ...
  <div className="settings-grid">
   <section className="panel">
    ...
    <div className="field">
     ...
     <SelectMenu id="trash-retention" value={String(trashRetention)} label="Keep deleted items"
      options={[{value:"14",label:"14 days"},{value:"30",label:"30 days"}]}
      onChange={value=>void saveTrashRetention(value==="14"?14:30)}/>
   ...
   <section className="panel">
    ...
    <div className="manage-list">
     {projects.rows.map(p=><div className="manage-row" key={p.id}>
      ...
      <SelectMenu className="input input-compact tint-select" value={p.color??""} label={`${p.name} colour`}
       options={colourOptions} onChange={value=>void projects.update(p.id,{color:value||null})}/>
      ...
      <button className="icon-button" title="Move project to Trash"
       onClick={()=>ask({title:"Move project to Trash?",message:`“${p.name}” and its project view can be restored from Trash. Linked work stays attached.`,confirmLabel:"Move to Trash"},()=>deleteProject(p.id))}>
       <Trash2/></button>
     </div>)}
   ...
   <section className="panel">
    ...
    <div className="manage-list">
     {tags.tags.map(t=><div className="manage-row" key={t.id}>
      ...
      <SelectMenu className="input input-compact tint-select" value={t.color??""} label={`${t.name} colour`}
       options={colourOptions} onChange={value=>void tags.recolor(t.id,value||null)}/>
      <small className="muted-note">{tags.usage.get(t.id)??0} uses</small>
      <button className="icon-button" title="Move tag to Trash"
       onClick={()=>ask({title:"Move tag to Trash?",message:`“${t.name}” can be restored with its record connections intact.`,confirmLabel:"Move to Trash"},()=>tags.destroy(t.id).then(()=>{}))}>
       <Trash2/></button>
     </div>)}
