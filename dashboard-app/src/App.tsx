...
import { BriefcaseBusiness, ChevronDown, CircleUserRound, FileText, FlaskConical, Home, LayoutDashboard, Menu, Plus, Search, Send, Trash2, X } from "lucide-react";
...
type TaskRow={id:string;title:string;status:Status;priority:"Low"|"Medium"|"High";due_at:string|null};
const toTask=(row:TaskRow):Task=>({id:row.id,title:row.title,status:row.status,priority:row.priority,project:"Inbox",due:row.due_at?new Date(row.due_at).toLocaleDateString():undefined});

function Workspace({userId}:{userId:string}){
 const [view,setView]=useState<View>("home"),[tasks,setTasks]=useState<Task[]>([]),[query,setQuery]=useState(""),[sidebar,setSidebar]=useState(false),[composer,setComposer]=useState(false),[newTask,setNewTask]=useState(""),[loading,setLoading]=useState(true),[notice,setNotice]=useState("");
 useEffect(()=>{let active=true;const load=async()=>{if(!supabase)return;const {data,error}=await supabase.from("tasks").select("id,title,status,priority,due_at").order("created_at",{ascending:false});if(!active)return;if(error)setNotice(error.message);else setTasks((data as TaskRow[]).map(toTask));setLoading(false)};void load();return()=>{active=false}},[]);
 const filtered=useMemo(()=>tasks.filter(t=>(t.title+" "+t.project).toLowerCase().includes(query.toLowerCase())),[tasks,query]);
 const nav=(v:View)=>{setView(v);setSidebar(false)};
 const move=async(id:string,status:Status)=>{if(!supabase)return;const previous=tasks;setTasks(v=>v.map(t=>t.id===id?{...t,status}:t));const {error}=await supabase.from("tasks").update({status,updated_at:new Date().toISOString()}).eq("id",id);if(error){setTasks(previous);setNotice(error.message)}};
 const add=async()=>{const title=newTask.trim();if(!title||!supabase)return;setNotice("");const {data,error}=await supabase.from("tasks").insert({user_id:userId,title,status:"Backlog",priority:"Medium",source:"dashboard"}).select("id,title,status,priority,due_at").single();if(error){setNotice(error.message);return}setTasks(v=>[toTask(data as TaskRow),...v]);setNewTask("");setComposer(false)};
 const remove=async(id:string)=>{if(!supabase||!window.confirm("Delete this task?"))return;const previous=tasks;setTasks(v=>v.filter(t=>t.id!==id));const {error}=await supabase.from("tasks").delete().eq("id",id);if(error){setTasks(previous);setNotice(error.message)}};
 return <div className="shell">
  ...
  </nav><div className="telegram"><Send size={18}/><div><strong>Telegram inbox</strong><small>Supabase task storage active</small></div></div></aside>
  <main><header><button className="mobile-menu" onClick={()=>setSidebar(!sidebar)}><Menu/></button><div className="crumb">Workspace / <strong>{labels[view]}</strong></div><div className="header-actions"><label className="search"><Search/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search workspace"/></label><button className="profile" onClick={()=>void supabase?.auth.signOut()} title="Sign out"><CircleUserRound/></button></div></header>
   ...
   {notice&&<p className="data-notice" role="alert">{notice}</p>}{loading&&<p className="data-loading">Loading your private tasks…</p>}
   {!loading&&view==="home"&&<HomeView tasks={filtered} open={nav}/>}
   {!loading&&view==="kanban"&&<Kanban tasks={filtered} move={move} remove={remove}/>}
  ...
  {composer&&<div className="modal-backdrop" onMouseDown={()=>setComposer(false)}><section className="modal" onMouseDown={e=>e.stopPropagation()}><button className="close" onClick={()=>setComposer(false)}><X/></button><p className="kicker">Private Supabase task</p><h2>Add to task inbox</h2><input autoFocus value={newTask} onChange={e=>setNewTask(e.target.value)} onKeyDown={e=>e.key==="Enter"&&void add()} placeholder="What needs doing?"/><div className="modal-actions"><button className="secondary" onClick={()=>setComposer(false)}>Cancel</button><button className="primary" onClick={()=>void add()}>Add task</button></div></section></div>}
 </div>
}
...
function Kanban({tasks,move,remove}:{tasks:Task[];move:(id:string,s:Status)=>void;remove:(id:string)=>void}){return <div className="board">{columns.map(s=><section className="column" key={s} onDragOver={e=>e.preventDefault()} onDrop={e=>void move(e.dataTransfer.getData("task"),s)}><div className="column-head"><span>{s}</span><b>{tasks.filter(t=>t.status===s).length}</b></div>{tasks.filter(t=>t.status===s).map(t=><article draggable onDragStart={e=>e.dataTransfer.setData("task",t.id)} className="task-card" key={t.id}><span className={"priority "+t.priority.toLowerCase()}/><button className="delete-card" onClick={()=>void remove(t.id)} title="Delete task"><Trash2/></button><h3>{t.title}</h3><p>{t.project}</p><footer><small>{t.due||"No date"}</small><Badge text={t.priority}/></footer></article>)}</section>)}</div>}
